...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $params['data'] = Category::where('status', 1)->orderby('sort_order', 'asc')->latest()->get();
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $delete = '<form class="delete-form" method="POST" action="' . route('admin.category.destroy', [$row->id]) . '">
                            <input type="hidden" name="_token" value="' . csrf_token() . '">
                            <input type="hidden" name="_method" value="DELETE">
                            <button type="submit" class="edit-delete-btn"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around">';
                        $btn .= '<a class="edit-delete-btn mr-2 edit_category" data-toggle="modal" href="#EditModel" data-id="' . $row->id . '" data-name="' . $row->name . '" data-image="' . $this->common->getImage($this->folder, $row['image'], 'normal', $row['storage_type']) . '" data-storage_type="' . $row->storage_type . '">';
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (isset($requestData['image'])) {
                $requestData['image'] = $this->common->saveImage($requestData['image'], $this->folder, 'category_', $requestData['storage_type']);
            } else {
                $requestData['image'] = "";
            }
            ...
            $data = Category::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_add_category')]);
            }
            return response()->json(['status' => 400, 'errors' => __('label.error_add_category')]);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
    public function update($id, Request $request)
    {
        try {
            ...
            $data = Category::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_edit_category')]);
            }
            return response()->json(['status' => 400, 'errors' => __('label.error_edit_category')]);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('admin.category.index')->with('success', __('label.category_delete'));
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
    ...
    public function CategorySortableSave(Request $request)
    {
        try {
            ...
            if (isset($ids) && $ids != null && $ids != "") {
                ...
                for ($i = 0; $i < count($id_array); $i++) {
                    Category::where('id', $id_array[$i])->update(['sort_order' => $i + 1]);
                }
            }
