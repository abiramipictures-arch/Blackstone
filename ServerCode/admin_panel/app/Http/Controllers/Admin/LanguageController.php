...
class LanguageController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $params['data'] = Language::where('status', 1)->orderby('sort_order', 'asc')->latest()->get();
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $delete = '<form class="delete-form" method="POST" action="' . route('admin.language.destroy', [$row->id]) . '">
                            <input type="hidden" name="_token" value="' . csrf_token() . '">
                            <input type="hidden" name="_method" value="DELETE">
                            <button type="submit" class="edit-delete-btn"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around">';
                        $btn .= '<a class="edit-delete-btn mr-2 edit_language" data-toggle="modal" href="#EditModel" data-id="' . $row->id . '" data-name="' . $row->name . '" data-image="' . $this->common->getImage($this->folder, $row['image'], 'normal', $row['storage_type']) . '" data-storage_type="' . $row->storage_type . '">';
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (isset($requestData['image'])) {
                $requestData['image'] = $this->common->saveImage($requestData['image'], $this->folder, 'lang_', $requestData['storage_type']);
            } else {
                $requestData['image'] = "";
            }
            ...
            $data = Language::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
