    ...
    <div class="right-content">
        ...
        <div class="body-content">
            ...
            <div class="row">
                ...
                <div class="col-sm-2">
                    <a href="{{ route('producer.tvshow.episode.add', ['tvshow_id' => $tvshow_id, 'type_id' => $type['id']]) }}" class="btn btn-default-white mw-150">
    ...
    <script>
        $(document).ready(function () {
            var table = $('#datatable').DataTable({
                ...dataTableDefaults,
                ajax: {
                    url: "{{ route('producer.tvshow.episode.index', ['tvshow_id' => $tvshow_id, 'type_id' => $type['id']]) }}",
                    data: function (d) {
        ...
        function change_status(id) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('producer.tvshow.episode.status') }}",
        ...
        function save_episode_sortable() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("producer.tvshow.episode.sortable") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function (resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_episode_sortable', '{{ route("producer.tvshow.episode.index", ["tvshow_id" => $tvshow_id, "type_id" => $type["id"]]) }}');
                },
