            ...
            <div class="row mb-2">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('producer.shorts.episode.index', ['id' => $data['show_id'], 'type_id' => $type['id']]) }}">{{__('label.episodes')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_episode')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2">
                    <a href="{{ route('producer.shorts.episode.index', ['id' => $data['show_id'], 'type_id' => $type['id']]) }}" class="btn btn-default-white mw-120">{{__('label.episode_list')}}</a>
                </div>
            </div>
            ...
            <form id="edit_shorts_episode" enctype="multipart/form-data">
                ...
                <div class="card custom-border-card mb-3">
                    ...
                    <div class="card-footer">
                        <button type="button" class="btn btn-default mw-120" onclick="edit_shorts_episode()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.update')}}</button>
                        <a href="{{ route('producer.shorts.episode.index', ['id' => $data['show_id'], 'type_id' => $type['id']]) }}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
...
@section('pagescript')
    ...
    <script>
        ...
        function edit_shorts_episode() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("producer.shorts.episode.update", ["shorts_id" => $data["show_id"], "id" => $data->id]) }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'edit_shorts_episode', '{{ route("producer.shorts.episode.index", ["id" => $data["show_id"], "type_id" => $type["id"]]) }}');
                },
