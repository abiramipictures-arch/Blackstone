            ...
            <div class="row">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('admin.video.index', ['type_id' => $type['id']]) }}">{{ $type['name'] }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.add_content')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2">
                    <a href="{{ route('admin.video.index', ['type_id' => $type['id']]) }}" class="btn btn-default-white mw-120">{{__('label.content_list')}}</a>
                </div>
            </div>

            <form id="save_video" enctype="multipart/form-data">
                <input type="hidden" name="type_id" value="{{ $type['id'] }}">
                <input type="hidden" name="video_type" value="{{ $type['type'] }}">
                <!-- Title-Card -->
                <div class="card custom-border-card mb-3">
                    <div class="card-header"><i class="fa-solid fa-film"></i><span>{{__('label.add_content')}}</span></div>
                    <div class="card-body py-2">
                        ...
                        @elseif ($status == 1)
                            ...
                            <div class="form-row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <input type="text" name="name" id="Tmdb_name" list="Tmdb_name_list" class="form-control" placeholder="{{__('label.movies_name_here')}}">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="video_upload_type" id="video_upload_type" class="form-control">
                                        ...
                                        <option value="live_stream_url">{{__('label.live_stream_url')}}</option>
                ...
                <div class="card custom-border-card">
                    ...
                    <div class="card-footer">
                        <button type="button" class="btn btn-default mw-120" onclick="save_video()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.save')}}</button>
                        <a href="{{route('admin.video.index', ['type_id' => $type['id']])}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
        ...
        function save_video() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_video', '{{ route("admin.video.index",  ["type_id" => $type["id"]]) }}');
                },
