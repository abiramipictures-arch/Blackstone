...
@section('tab_title', __('label.category'))
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            ...
            <div class="card custom-border-card">
                <div class="card-header"><i class="fa-solid fa-plus-circle"></i>{{__('label.add_category')}}</div>
                <form id="save_category" enctype="multipart/form-data">
                    ...
                    <div class="card-footer">
                        <button type="button" class="btn btn-default mw-120" onclick="save_category()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.save')}}</button>
            ...
            <!-- Search & Table -->
            <div class="card custom-border-card">
                <div class="card-header"><i class="fa-solid fa-list"></i>{{__('label.category')}}</div>
                <div class="card-body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">{{__('label.edit_category')}}</h5>
                        ...
                        <form id="update_category" enctype="multipart/form-data">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default mw-120 mr-2" onclick="update_category()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.update')}}</button>
            ...
            <div class="modal fade" id="sortableModal" tabindex="-1" data-backdrop="static" role="dialog" aria-labelledby="sortableModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title w-100 text-center" id="sortableModalLabel">{{__('label.category_sortable_list')}}</h5>
                        ...
                        <div class="modal-footer justify-content-center">
                            <form id="save_category_sortable" enctype="multipart/form-data">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input id="outputvalues" type="hidden" name="ids" value="" />
                                <div class="w-100 text-center">
                                    <button type="button" class="btn btn-default mw-120" onclick="save_category_sortable()">
...
@section('pagescript')
    ...
    <script>
        ...
        function save_category() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_category', '{{ route("admin.category.index") }}');
                },
        ...
        function update_category() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    get_responce_message(resp, 'update_category', '{{ route("admin.category.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
        ...
        var category_data = JSON.parse('<?php echo json_encode($data); ?>'); // Convert PHP to JS array
        function sortableBTN() {
            ...
            for (var i = 0; i < category_data.length; i++) { 
                htmlContent += `<div id="${category_data[i].id}" class=" bn-sort-item listitemClass">
                                    <i class="fa-solid fa-grip-vertical drag-handle"></i>
                                    <span class="sort-num">${i + 1}</span>
                                    <span style="flex:1">${category_data[i].name}</span>                                    
                                </div>`;
            }                    
            listContainer.append(htmlContent);
        }
        function save_category_sortable() {
            if ('{{ Demo_Mode() }}' != 1) {
                showError();
                return;
            }
            ...
            $("#dvloader").show();
            var formData = new FormData($("#save_category_sortable")[0]);
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.category.sortable.save") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_category_sortable', '{{ route("admin.category.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
