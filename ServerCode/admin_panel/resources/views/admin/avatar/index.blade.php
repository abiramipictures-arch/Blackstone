...
@section('tab_title', __('label.avatar'))
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            ...
            <div class="card custom-border-card">
                <div class="card-header"><i class="fa-solid fa-plus-circle"></i>{{__('label.add_avatar')}}</div>
                <form id="save_avatar" enctype="multipart/form-data">
                    ...
                    <div class="card-footer">
                        <button type="button" class="btn btn-default mw-120" onclick="save_avatar()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.save')}}</button>
            ...
            <!-- Search & Table -->
            <div class="card custom-border-card">
                <div class="card-header"><i class="fa-solid fa-list"></i>{{__('label.avatar')}}</div>
                <div class="card-body">
                    ...
                    <div class="modal-content">
                        ...
                        <form id="update_avatar" enctype="multipart/form-data">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default mw-120 mr-2" onclick="update_avatar()"><i class="fa-solid fa-floppy-disk fa-lg mr-2"></i>{{__('label.update')}}</button>
            ...
            <div class="modal fade" id="sortableModal" tabindex="-1" data-backdrop="static" role="dialog" aria-labelledby="sortableModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title w-100 text-center" id="sortableModalLabel">{{__('label.avatar_sortable_list')}}</h5>
                        ...
                        <div class="modal-footer justify-content-center">
                            <form id="save_avatar_sortable" enctype="multipart/form-data">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input id="outputvalues" type="hidden" name="ids" value="" />
                                <div class="w-100 text-center">
                                    <button type="button" class="btn btn-default mw-120" onclick="save_avatar_sortable()">
...
@section('pagescript')
    ...
    <script>
        ...
        function save_avatar() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_avatar', '{{ route("admin.avatar.index") }}');
                },
        ...
        function change_status(id, status) {
            ...
            $.ajax({
                type: "GET",
                url: "{{route('admin.avatar.show', '')}}" + "/" + id,
                headers: {
        ...
        function update_avatar() {
            ...
            $("#dvloader").show();
            var formData = new FormData($("#update_avatar")[0]);
            var Edit_Id = $("#edit_id").val();

            var url = '{{ route("admin.avatar.update", ":id") }}';
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    get_responce_message(resp, 'update_avatar', '{{ route("admin.avatar.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
        ...
        var avatar_data = JSON.parse('<?php echo json_encode($data); ?>'); // Convert PHP to JS array
        function sortableBTN() {
            var listContainer = $('#contentListId');
            listContainer.html('');

            var htmlContent = ''; 
            for (var i = 0; i < avatar_data.length; i++) { 
                htmlContent += `<div id="${avatar_data[i].id}" class=" bn-sort-item listitemClass">
                                    <i class="fa-solid fa-grip-vertical drag-handle"></i>
                                    <span class="sort-num">${i + 1}</span>
                                    <span style="flex:1">${avatar_data[i].name}</span>                                    
                                </div>`;
            }                    
            listContainer.append(htmlContent);
        }
        function save_avatar_sortable() {
            if ('{{ Demo_Mode() }}' != 1) {
                showError();
                return;
            }
            ...
            $("#dvloader").show();
            var formData = new FormData($("#save_avatar_sortable")[0]);
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.avatar.sortable.save") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_avatar_sortable', '{{ route("admin.avatar.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
